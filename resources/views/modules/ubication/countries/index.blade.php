        ...
        <div class="page-header">
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <h1 class="page-header-title">Listado de países</h1>
                </div>
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('countries.create') }}">
                        <i class="bi-plus-circle-fill me-1"></i> Agregar país
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">Total países registrados</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span class="js-counter display-4 text-dark">{{ $totalCountries ?? 0 }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">Países activos</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span class="js-counter display-4 text-dark">{{ $activeCountries ?? 0 }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5">
                <div class="card h-100">
                    <div class="card-body">
                        <h6 class="card-subtitle mb-2">Países inactivos</h6>
                        <div class="row align-items-center gx-2">
                            <div class="col">
                                <span class="js-counter display-4 text-dark">{{ $inactiveCountries ?? 0 }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                ...
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    @php
                        $activeFilters = count(array_filter(request()->only(['search', 'status'])));
                    @endphp
                    <div class="dropdown">
                                ...
                                <div class="card-body">
                                    <form action="{{ route('countries.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('countries.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
                ...
                <table id="datatable"
                    ...
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th class="table-column-ps-0">País</th>
                            <th>Estado</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="table-column-ps-0">
                                <span class="d-block h5 text-inherit mb-0">{{ $country->name }}</span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    ...
                                    @if ($country->is_active)
                                    <form action="{{ route('countries.destroy', $country->id) }}" method="POST" class="requires-confirmation" data-message="¿Desactivar el país {{ addslashes($country->name) }}? Esto también desactivará todos sus departamentos y municipios asociados.">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    <form action="{{ route('countries.restore', $country->id) }}" method="POST" class="requires-confirmation" data-message="¿Reactivar el país {{ addslashes($country->name) }}? Esto también reactivará todos sus departamentos y municipios asociados.">
                                        @csrf
                        ...
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <div class="tom-select-custom">
                                <select id="datatableEntries"
                                    ...
                                    onchange="window.location.href = '{{ route('countries.index', request()->except(['per_page', 'page'])) }}' + ( '{{ route('countries.index', request()->except(['per_page', 'page'])) }}'.includes('?') ? '&' : '?' ) + 'per_page=' + this.value">
                            ...
                            <span id="datatableWithPaginationInfoTotalQty">{{ $countries->total() }}</span>
                        </div>
                    </div>
                    <div class="col-sm-auto">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const checkboxes = document.querySelectorAll('input[id^="countriesDataCheck"]');
        ...
        if (bulkDeleteBtn) {
            bulkDeleteBtn.addEventListener('click', function() {
                if (confirm('¿Estás seguro de que deseas eliminar los países seleccionados? Se desactivarán en cascada sus departamentos y municipios asociados.')) {
                    sendBulkRequest('{{ route("countries.destroy-multiple") }}');
        ...
        if (bulkRestoreBtn) {
            bulkRestoreBtn.addEventListener('click', function() {
                if (confirm('¿Estás seguro de que deseas reactivar los países seleccionados? Se reactivarán en cascada.')) {
                    sendBulkRequest('{{ route("countries.restore-multiple") }}');
                    sessionStorage.removeItem(storageKey);
                }
            });
        }
        ...
        if (exportExcelBtn) exportExcelBtn.addEventListener('click', () => sendBulkRequest('{{ route("countries.export.excel") }}', 'excel'));
        
        const exportCsvBtn = document.getElementById('export-csv');
        if (exportCsvBtn) exportCsvBtn.addEventListener('click', () => sendBulkRequest('{{ route("countries.export.csv") }}', 'csv'));
        
        const exportPdfBtn = document.getElementById('export-pdf');
        if (exportPdfBtn) exportPdfBtn.addEventListener('click', () => sendBulkRequest('{{ route("countries.export.pdf") }}', 'pdf'));
        
        const exportPrintBtn = document.getElementById('export-print');
        if (exportPrintBtn) exportPrintBtn.addEventListener('click', () => sendBulkRequest('{{ route("countries.print") }}', 'print'));
    });
</script>
@endpush