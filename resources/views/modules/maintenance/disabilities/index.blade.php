            ...
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb"><ol class="breadcrumb breadcrumb-no-gutter">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Discapacidades</li>
                    </ol></nav>
                    <h1 class="page-header-title">Listado de discapacidades</h1>
                </div>
                <div class="col-sm-auto"><a class="btn btn-primary" href="{{ route('maintenance.disabilities.create') }}"><i class="bi-plus-circle-fill me-1"></i> Agregar discapacidad</a></div>
            </div>
        </div>
            @if (session('success'))
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('maintenance.disabilities.index') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            @if(request('search'))<a class="input-group-append input-group-text text-muted" href="{{ route('maintenance.disabilities.index', request()->except('search')) }}"><i class="bi-x-lg"></i></a>@endif
                        </div>
                    </form>
                </div>
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm-end dropdown-card card-dropdown-filter-centered" style="min-width: 22rem;">
                            <div class="card"><div class="card-header card-header-content-between"><h5 class="card-header-title">Filtrar discapacidades</h5></div>
                                <div class="card-body"><form action="{{ route('maintenance.disabilities.index') }}" method="GET">
                                    ...
                                    <div class="d-grid gap-2"><button type="submit" class="btn btn-primary">Aplicar</button><a class="btn btn-white" href="{{ route('maintenance.disabilities.index') }}">Limpiar</a></div>
                                </form></div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="table-responsive datatable-custom position-relative">
                <table id="datatable" class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <thead class="thead-light"><tr>
                        ...
                        <th class="table-column-ps-0">Discapacidad</th><th>Estado</th><th>Acciones</th>
                    </tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><div class="d-flex gap-1">
                                <a href="{{ route('maintenance.disabilities.edit', $item->id) }}" class="btn btn-white btn-sm" title="Editar"><i class="bi-pencil-fill"></i></a>
                                @if($item->is_active)
                                    <form action="{{ route('maintenance.disabilities.destroy', $item->id) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn btn-white btn-sm" title="Desactivar"><i class="bi-trash"></i></button></form>
                                @else
                                    <form action="{{ route('maintenance.disabilities.restore', $item->id) }}" method="POST">@csrf<button type="submit" class="btn btn-white btn-sm" title="Reactivar"><i class="bi-arrow-counterclockwise"></i></button></form>
                                @endif
                            </div></td>
                        </tr>
                        @empty
                        <tr><td colspan="4" class="text-center">No hay discapacidades registradas.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <div class="row justify-content-center justify-content-sm-between align-items-sm-center">
                    <div class="col-sm mb-2 mb-sm-0"><div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                        ...
                        <div class="tom-select-custom"><select id="datatableEntries" class="js-select form-select form-select-borderless w-auto" autocomplete="off" data-hs-tom-select-options='{"searchInDropdown":false,"hideSearch":true}'
                            onchange="window.location.href='{{ route('maintenance.disabilities.index', request()->except(['per_page','page'])) }}'+(('{{ route('maintenance.disabilities.index', request()->except(['per_page','page'])) }}'.includes('?'))?'&':'?')+'per_page='+this.value">
...
document.addEventListener('DOMContentLoaded',function(){
    const storageKey='selectedDisabilities';let selectedIds=JSON.parse(sessionStorage.getItem(storageKey)||'[]');
    ...
    document.getElementById('bulk-delete')?.addEventListener('click',()=>{sendBulkRequest('{{ route("maintenance.disabilities.destroy-multiple") }}');sessionStorage.removeItem(storageKey);});
    document.getElementById('bulk-restore')?.addEventListener('click',()=>{sendBulkRequest('{{ route("maintenance.disabilities.restore-multiple") }}');sessionStorage.removeItem(storageKey);});
    document.getElementById('export-excel')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.disabilities.export.excel") }}','excel'));
    document.getElementById('export-csv')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.disabilities.export.csv") }}','csv'));
    document.getElementById('export-pdf')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.disabilities.export.pdf") }}','pdf'));
    document.getElementById('export-print')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.disabilities.print") }}','print'));
});
</script>
@endpush
