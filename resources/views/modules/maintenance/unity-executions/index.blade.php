                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Unidades Ejecutoras</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">Listado de unidades ejecutoras</h1>
                </div>
            </div>
        </div>
            @if (session('success'))
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('unity-executions.index') }}" method="GET">
                        <div class="input-group input-group-merge input-group-flush">
                            <div class="input-group-prepend input-group-text bg-transparent border-0">
                                <i class="bi-search"></i>
                            </div>
                            <input name="search" type="text" class="form-control" placeholder="Buscar unidad" value="{{ request('search') }}">
                        </div>
                    </form>
                </div>
                                ...
                                <div class="card-body">
                                    <form action="{{ route('unity-executions.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('unity-executions.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <div class="table-responsive datatable-custom position-relative">
                <table id="datatable" class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                    ...
                    <tbody>
                        @forelse($unityExecutions as $item)
                        <tr>
                            <td>
                ...
                <div class="row justify-content-center justify-content-sm-between align-items-sm-center">
                    <div class="col-sm mb-2 mb-sm-0">
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <span class="text-secondary">{{ $unityExecutions->count() }}</span>
                            <span class="text-secondary mx-2">de</span>
                            <span>{{ $unityExecutions->total() }}</span>
                        </div>
                    </div>
                    <div class="col-sm-auto">
                        <div class="d-flex justify-content-center justify-content-sm-end">
                            {{ $unityExecutions->links('vendor.pagination.bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (exportExcelBtn) exportExcelBtn.addEventListener('click', () => sendExportRequest('{{ route("unity-executions.export.excel") }}'));

        const exportPdfBtn = document.getElementById('export-pdf');
        if (exportPdfBtn) exportPdfBtn.addEventListener('click', () => sendExportRequest('{{ route("unity-executions.export.pdf") }}'));

        const exportPrintBtn = document.getElementById('export-print');
        if (exportPrintBtn) exportPrintBtn.addEventListener('click', () => sendExportRequest('{{ route("unity-executions.print") }}'));
    });
</script>
@endpush
