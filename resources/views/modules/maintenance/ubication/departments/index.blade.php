        ...
        <div class="page-header">
            <div class="row align-items-end">
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('maintenance.departments.create') }}">
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('maintenance.departments.index') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            <input name="search" type="text" class="form-control" placeholder="Buscar" aria-label="Buscar departamentos" value="{{ request('search') }}">
                            @if(request('search'))
                            <a class="input-group-append input-group-text text-muted" href="{{ route('maintenance.departments.index', request()->except('search')) }}">
                                <i class="bi-x-lg"></i>
                            </a>
                            @endif
                        </div>
                ...
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    <div class="dropdown">
                        <button type="button" class="btn btn-white btn-sm dropdown-toggle w-100" id="departmentsExportDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                    ...
                    @php
                        $activeFilters = count(array_filter(request()->only(['country_id', 'status'])));
                    @endphp
                    <div class="dropdown">
                        <button type="button" class="btn btn-white btn-sm w-100" id="departmentsFilterDropdown" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm-end dropdown-card card-dropdown-filter-centered" aria-labelledby="departmentsFilterDropdown" style="min-width: 22rem;">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form action="{{ route('maintenance.departments.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('maintenance.departments.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="table-column-pe-0">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" value="{{ $department->id }}" id="departmentsDataCheck{{ $department->id }}">
                            ...
                            <td>
                                <div class="d-flex gap-2">
                                    <a href="{{ route('maintenance.departments.edit', $department->id) }}" class="btn btn-white btn-sm">
                                        <i class="bi-pencil-fill me-1"></i> Editar
                                    </a>
                                    @if ($department->is_active)
                                    <form action="{{ route('maintenance.departments.destroy', $department->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    <form action="{{ route('maintenance.departments.restore', $department->id) }}" method="POST">
                                        @csrf
                ...
                <div class="row justify-content-center justify-content-sm-between align-items-sm-center">
                    <div class="col-sm mb-2 mb-sm-0">
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <div class="tom-select-custom">
                                <select id="datatableEntries" class="js-select form-select form-select-borderless w-auto" autocomplete="off" data-hs-tom-select-options='{"searchInDropdown": false, "hideSearch": true}' onchange="window.location.href = '{{ route('maintenance.departments.index', request()->except(['per_page', 'page'])) }}' + ( '{{ route('maintenance.departments.index', request()->except(['per_page', 'page'])) }}'.includes('?') ? '&' : '?' ) + 'per_page=' + this.value">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (bulkDeleteBtn) {
            bulkDeleteBtn.addEventListener('click', function() {
                sendBulkRequest('{{ route("maintenance.departments.destroy-multiple") }}');
        ...
        if (bulkRestoreBtn) {
            bulkRestoreBtn.addEventListener('click', function() {
                sendBulkRequest('{{ route("maintenance.departments.restore-multiple") }}');
                sessionStorage.removeItem(storageKey);
            });
        }

        const exportExcelBtn = document.getElementById('export-excel');
        if (exportExcelBtn) exportExcelBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.departments.export.excel") }}', 'excel'));
        
        const exportCsvBtn = document.getElementById('export-csv');
        if (exportCsvBtn) exportCsvBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.departments.export.csv") }}', 'csv'));
        
        const exportPdfBtn = document.getElementById('export-pdf');
        if (exportPdfBtn) exportPdfBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.departments.export.pdf") }}', 'pdf'));
        
        const exportPrintBtn = document.getElementById('export-print');
        if (exportPrintBtn) exportPrintBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.departments.print") }}', 'print'));
    });
</script>
@endpush
