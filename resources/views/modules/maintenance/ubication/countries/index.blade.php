        ...
        <div class="page-header">
            <div class="row align-items-end">
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('maintenance.countries.create') }}">
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('maintenance.countries.index') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            @if(request('search'))
                            <a class="input-group-append input-group-text text-muted" href="{{ route('maintenance.countries.index', request()->except('search')) }}">
                                <i class="bi-x-lg"></i>
                            </a>
                            @endif
                        </div>
                                ...
                                <div class="card-body">
                                    <form action="{{ route('maintenance.countries.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('maintenance.countries.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('maintenance.countries.edit', $country->id) }}" class="btn btn-white btn-sm" title="Editar">
                                        <i class="bi-pencil-fill"></i>
                                    </a>
                                    @if ($country->is_active)
                                    <form action="{{ route('maintenance.countries.destroy', $country->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    <form action="{{ route('maintenance.countries.restore', $country->id) }}" method="POST">
                                        @csrf
                        ...
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <div class="tom-select-custom">
                                <select id="datatableEntries"
                                    ...
                                    onchange="window.location.href = '{{ route('maintenance.countries.index', request()->except(['per_page', 'page'])) }}' + ( '{{ route('maintenance.countries.index', request()->except(['per_page', 'page'])) }}'.includes('?') ? '&' : '?' ) + 'per_page=' + this.value">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (bulkDeleteBtn) {
            bulkDeleteBtn.addEventListener('click', function() {
                sendBulkRequest('{{ route("maintenance.countries.destroy-multiple") }}');
        ...
        if (bulkRestoreBtn) {
            bulkRestoreBtn.addEventListener('click', function() {
                sendBulkRequest('{{ route("maintenance.countries.restore-multiple") }}');
                sessionStorage.removeItem(storageKey);
            });
        }

        const exportExcelBtn = document.getElementById('export-excel');
        if (exportExcelBtn) exportExcelBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.countries.export.excel") }}', 'excel'));
        
        const exportCsvBtn = document.getElementById('export-csv');
        if (exportCsvBtn) exportCsvBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.countries.export.csv") }}', 'csv'));
        
        const exportPdfBtn = document.getElementById('export-pdf');
        if (exportPdfBtn) exportPdfBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.countries.export.pdf") }}', 'pdf'));
        
        const exportPrintBtn = document.getElementById('export-print');
        if (exportPrintBtn) exportPrintBtn.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.countries.print") }}', 'print'));
    });
</script>
@endpush
