...
@section('title', 'Departamentos de Trabajo')
@section('content')
                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Departamentos de Trabajo</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">Listado de departamentos de trabajo</h1>
                </div>
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('work-departments.create') }}">
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('work-departments.index') }}" method="GET">
                        <div class="input-group input-group-merge input-group-flush">
                            <div class="input-group-prepend input-group-text bg-transparent border-0">
                                <i class="bi-search"></i>
                            </div>
                            <input name="search" type="text" class="form-control" placeholder="Buscar departamento" value="{{ request('search') }}">
                        </div>
                    </form>
                </div>
                                ...
                                <div class="card-body">
                                    <form action="{{ route('work-departments.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('work-departments.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <div class="table-responsive datatable-custom position-relative">
                <table id="datatable" class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                    ...
                    <tbody>
                        @forelse($workDepartments as $item)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('work-departments.edit', $item->id) }}" class="btn btn-white btn-sm" title="Editar">
                                        <i class="bi-pencil-fill"></i>
                                    </a>
                                    @if ($item->is_active)
                                    <form action="{{ route('work-departments.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    <form action="{{ route('work-departments.restore', $item->id) }}" method="POST">
                                        @csrf
                ...
                <div class="row justify-content-center justify-content-sm-between align-items-sm-center">
                    <div class="col-sm mb-2 mb-sm-0">
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <span class="text-secondary">{{ $workDepartments->count() }}</span>
                            <span class="text-secondary mx-2">de</span>
                            <span>{{ $workDepartments->total() }}</span>
                        </div>
                    </div>
                    <div class="col-sm-auto">
                        <div class="d-flex justify-content-center justify-content-sm-end">
                            {{ $workDepartments->links('vendor.pagination.bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (bulkDeleteBtn) bulkDeleteBtn.addEventListener('click', () => sendBulkRequest('{{ route("work-departments.destroy-multiple") }}'));

        const bulkRestoreBtn = document.getElementById('bulk-restore');
        if (bulkRestoreBtn) bulkRestoreBtn.addEventListener('click', () => sendBulkRequest('{{ route("work-departments.restore-multiple") }}'));

        const exportExcelBtn = document.getElementById('export-excel');
        if (exportExcelBtn) exportExcelBtn.addEventListener('click', () => sendBulkRequest('{{ route("work-departments.export.excel") }}', 'excel'));

        const exportPdfBtn = document.getElementById('export-pdf');
        if (exportPdfBtn) exportPdfBtn.addEventListener('click', () => sendBulkRequest('{{ route("work-departments.export.pdf") }}', 'pdf'));

        const exportPrintBtn = document.getElementById('export-print');
        if (exportPrintBtn) exportPrintBtn.addEventListener('click', () => sendBulkRequest('{{ route("work-departments.print") }}', 'print'));
