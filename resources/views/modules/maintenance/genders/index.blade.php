...
<main id="content" role="main" class="main">
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-end">
                <div class="col-sm mb-2 mb-sm-0">
                    ...
                    <h1 class="page-header-title">Listado de géneros</h1>
                </div>
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('maintenance.genders.create') }}"><i class="bi-plus-circle-fill me-1"></i> Agregar género</a>
                </div>
            </div>
        </div>
            @if (session('success'))
        ...
        <div class="row">
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5"><div class="card h-100"><div class="card-body"><h6 class="card-subtitle mb-2">Total registrados</h6><span class="js-counter display-4 text-dark">{{ $total ?? 0 }}</span></div></div></div>
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5"><div class="card h-100"><div class="card-body"><h6 class="card-subtitle mb-2">Activos</h6><span class="js-counter display-4 text-dark">{{ $active ?? 0 }}</span></div></div></div>
            <div class="col-sm-6 col-lg-4 mb-3 mb-lg-5"><div class="card h-100"><div class="card-body"><h6 class="card-subtitle mb-2">Inactivos</h6><span class="js-counter display-4 text-dark">{{ $inactive ?? 0 }}</span></div></div></div>
        </div>
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('maintenance.genders.index') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            @if(request('search'))<a class="input-group-append input-group-text text-muted" href="{{ route('maintenance.genders.index', request()->except('search')) }}"><i class="bi-x-lg"></i></a>@endif
                        </div>
                    </form>
                </div>
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm-end" aria-labelledby="exportDd">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm-end dropdown-card card-dropdown-filter-centered" aria-labelledby="filterDd" style="min-width: 22rem;">
                            <div class="card">
                                <div class="card-header card-header-content-between"><h5 class="card-header-title">Filtrar géneros</h5></div>
                                <div class="card-body">
                                    <form action="{{ route('maintenance.genders.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2"><button type="submit" class="btn btn-primary">Aplicar</button><a class="btn btn-white" href="{{ route('maintenance.genders.index') }}">Limpiar</a></div>
                                    </form>
                                </div>
                            </div>
                        </div>
                    ...
                    <span id="selectedCountWrapper" class="text-body small align-items-center gap-2 ms-2" style="display: flex; display: none !important; border-left: 1px solid #e7eaf3; padding-left: .5rem;">
                        ...
                        <div class="form-check form-check-sm mb-0" title="Seleccionar todos ({{ count($allFilteredIds) }})">
                        ...
                        <button type="button" id="clearSelection" class="btn btn-link btn-sm p-0 ms-1 text-muted" style="line-height: 1; font-size: 0.75rem;" title="Limpiar selección"><i class="bi-x-lg"></i></button>
                    </span>
                </div>
            </div>
                ...
                <table id="datatable" class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th class="table-column-ps-0">Género</th>
                            <th>Estado</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><div class="d-flex gap-1">
                                <a href="{{ route('maintenance.genders.edit', $item->id) }}" class="btn btn-white btn-sm" title="Editar"><i class="bi-pencil-fill"></i></a>
                                @if ($item->is_active)
                                <form action="{{ route('maintenance.genders.destroy', $item->id) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn btn-white btn-sm" title="Desactivar"><i class="bi-trash"></i></button></form>
                                @else
                                <form action="{{ route('maintenance.genders.restore', $item->id) }}" method="POST">@csrf<button type="submit" class="btn btn-white btn-sm" title="Reactivar"><i class="bi-arrow-counterclockwise"></i></button></form>
                                @endif
                            </div></td>
                        </tr>
                        @empty
                        <tr><td colspan="4" class="text-center">No hay géneros registrados.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                        ...
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <div class="tom-select-custom">
                                <select id="datatableEntries" class="js-select form-select form-select-borderless w-auto" autocomplete="off" data-hs-tom-select-options='{"searchInDropdown": false, "hideSearch": true}'
                                    onchange="window.location.href = '{{ route('maintenance.genders.index', request()->except(['per_page', 'page'])) }}' + ( '{{ route('maintenance.genders.index', request()->except(['per_page', 'page'])) }}'.includes('?') ? '&' : '?' ) + 'per_page=' + this.value">
...
document.addEventListener('DOMContentLoaded', function () {
    const storageKey = 'selectedGenders';
    ...
    function updateUI() {
        ...
        if (selectedIds.length > 0) { countWrapper.style.setProperty('display', 'flex', 'important'); actionsWrapper.style.display = 'block'; }
        else { countWrapper.style.setProperty('display', 'none', 'important'); actionsWrapper.style.display = 'none'; }
        if (checkboxes.length > 0 && checkAll) checkAll.checked = Array.from(checkboxes).every(cb => cb.checked);
        sessionStorage.setItem(storageKey, JSON.stringify(selectedIds));
    }
    checkboxes.forEach(cb => {
        if (selectedIds.includes(cb.value)) cb.checked = true;
        cb.addEventListener('change', function() { if (this.checked) { if (!selectedIds.includes(this.value)) selectedIds.push(this.value); } else { selectedIds = selectedIds.filter(id => id !== this.value); } updateUI(); });
    });
    ...
    const btnAll = document.getElementById('selectAllFiltered');
    ...
    updateUI();
    function sendBulkRequest(url, fmt = null) {
        ...
        const ids = document.createElement('input'); ids.type='hidden'; ids.name='ids'; ids.value=JSON.stringify(selectedIds); form.appendChild(ids);
        if (fmt) { const f = document.createElement('input'); f.type='hidden'; f.name='format'; f.value=fmt; form.appendChild(f); }
        document.body.appendChild(form); form.submit();
    }
    document.getElementById('bulk-delete')?.addEventListener('click', () => { sendBulkRequest('{{ route("maintenance.genders.destroy-multiple") }}'); sessionStorage.removeItem(storageKey); });
    document.getElementById('bulk-restore')?.addEventListener('click', () => { sendBulkRequest('{{ route("maintenance.genders.restore-multiple") }}'); sessionStorage.removeItem(storageKey); });
    document.getElementById('export-excel')?.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.genders.export.excel") }}', 'excel'));
    document.getElementById('export-csv')?.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.genders.export.csv") }}', 'csv'));
    document.getElementById('export-pdf')?.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.genders.export.pdf") }}', 'pdf'));
    document.getElementById('export-print')?.addEventListener('click', () => sendBulkRequest('{{ route("maintenance.genders.print") }}', 'print'));
});
</script>
@endpush
