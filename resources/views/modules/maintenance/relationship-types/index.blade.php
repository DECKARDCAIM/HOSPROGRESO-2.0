...
@section('title', 'Tipos de Relación')
@section('content')
                ...
                <div class="col-sm mb-2 mb-sm-0">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb breadcrumb-no-gutter">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Tipos de Relación</li>
                        </ol>
                    </nav>
                    <h1 class="page-header-title">Listado de tipos de relación</h1>
                </div>
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('relationship-types.create') }}">
                        <i class="bi-plus-circle-fill me-1"></i> Agregar tipo
                    </a>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('relationship-types.index') }}" method="GET">
                        <div class="input-group input-group-merge input-group-flush">
                            <div class="input-group-prepend input-group-text bg-transparent border-0">
                                <i class="bi-search"></i>
                            </div>
                            <input name="search" type="text" class="form-control" placeholder="Buscar tipo" value="{{ request('search') }}">
                        </div>
                    </form>
                </div>
                                ...
                                <div class="card-body">
                                    <form action="{{ route('relationship-types.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('relationship-types.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <div class="table-responsive datatable-custom position-relative">
                <table id="datatable" class="table table-lg table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                    ...
                    <tbody>
                        @forelse($relationshipTypes as $item)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('relationship-types.edit', $item->id) }}" class="btn btn-white btn-sm" title="Editar">
                                        <i class="bi-pencil-fill"></i>
                                    </a>
                                    @if ($item->is_active)
                                    <form action="{{ route('relationship-types.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    <form action="{{ route('relationship-types.restore', $item->id) }}" method="POST">
                                        @csrf
                        ...
                        @empty
                        <tr>
                            <td colspan="4" class="text-center">No hay registros encontrados.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="row justify-content-center justify-content-sm-between align-items-sm-center">
                    <div class="col-sm mb-2 mb-sm-0">
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <span class="text-secondary">{{ $relationshipTypes->count() }}</span>
                            <span class="text-secondary mx-2">de</span>
                            <span>{{ $relationshipTypes->total() }}</span>
                        </div>
                    </div>
                    <div class="col-sm-auto">
                        <div class="d-flex justify-content-center justify-content-sm-end">
                            {{ $relationshipTypes->links('vendor.pagination.bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (bulkDeleteBtn) bulkDeleteBtn.addEventListener('click', () => sendBulkRequest('{{ route("relationship-types.destroy-multiple") }}'));

        const bulkRestoreBtn = document.getElementById('bulk-restore');
        if (bulkRestoreBtn) bulkRestoreBtn.addEventListener('click', () => sendBulkRequest('{{ route("relationship-types.restore-multiple") }}'));

        const exportExcelBtn = document.getElementById('export-excel');
        if (exportExcelBtn) exportExcelBtn.addEventListener('click', () => sendBulkRequest('{{ route("relationship-types.export.excel") }}', 'excel'));

        const exportPdfBtn = document.getElementById('export-pdf');
        if (exportPdfBtn) exportPdfBtn.addEventListener('click', () => sendBulkRequest('{{ route("relationship-types.export.pdf") }}', 'pdf'));

        const exportPrintBtn = document.getElementById('export-print');
        if (exportPrintBtn) exportPrintBtn.addEventListener('click', () => sendBulkRequest('{{ route("relationship-types.print") }}', 'print'));
