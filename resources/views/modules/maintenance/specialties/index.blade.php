    ...
    <div class="content container-fluid">
        <div class="page-header">
            <div class="row align-items-end">
                ...
                <div class="col-sm-auto"><a class="btn btn-primary" href="{{ route('maintenance.specialties.create') }}"><i class="bi-plus-circle-fill me-1"></i> Agregar especialidad</a></div>
            </div>
        </div>
            @if (session('success'))
        ...
        <div class="card">
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form action="{{ route('maintenance.specialties.index') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            @if(request('search'))<a class="input-group-append input-group-text text-muted" href="{{ route('maintenance.specialties.index', request()->except('search')) }}"><i class="bi-x-lg"></i></a>@endif
                        </div>
                    </form>
                </div>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm-end dropdown-card card-dropdown-filter-centered" style="min-width: 22rem;">
                            <div class="card"><div class="card-header card-header-content-between"><h5 class="card-header-title">Filtrar especialidades</h5></div>
                                <div class="card-body"><form action="{{ route('maintenance.specialties.index') }}" method="GET">
                                    ...
                                    <div class="d-grid gap-2"><button type="submit" class="btn btn-primary">Aplicar</button><a class="btn btn-white" href="{{ route('maintenance.specialties.index') }}">Limpiar</a></div>
                                </form></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><div class="d-flex gap-1">
                                <a href="{{ route('maintenance.specialties.edit', $item->id) }}" class="btn btn-white btn-sm" title="Editar"><i class="bi-pencil-fill"></i></a>
                                @if($item->is_active)
                                    <form action="{{ route('maintenance.specialties.destroy', $item->id) }}" method="POST">@csrf @method('DELETE')<button type="submit" class="btn btn-white btn-sm" title="Desactivar"><i class="bi-trash"></i></button></form>
                                @else
                                    <form action="{{ route('maintenance.specialties.restore', $item->id) }}" method="POST">@csrf<button type="submit" class="btn btn-white btn-sm" title="Reactivar"><i class="bi-arrow-counterclockwise"></i></button></form>
                                @endif
                            </div></td>
                        </tr>
                        @empty
                ...
                <div class="row justify-content-center justify-content-sm-between align-items-sm-center">
                    <div class="col-sm mb-2 mb-sm-0"><div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                        ...
                        <div class="tom-select-custom"><select id="datatableEntries" class="js-select form-select form-select-borderless w-auto" autocomplete="off" data-hs-tom-select-options='{"searchInDropdown":false,"hideSearch":true}'
                            onchange="window.location.href='{{ route('maintenance.specialties.index', request()->except(['per_page','page'])) }}'+(('{{ route('maintenance.specialties.index', request()->except(['per_page','page'])) }}'.includes('?'))?'&':'?')+'per_page='+this.value">
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    document.getElementById('bulk-delete')?.addEventListener('click',()=>{sendBulkRequest('{{ route("maintenance.specialties.destroy-multiple") }}');sessionStorage.removeItem(storageKey);});
    document.getElementById('bulk-restore')?.addEventListener('click',()=>{sendBulkRequest('{{ route("maintenance.specialties.restore-multiple") }}');sessionStorage.removeItem(storageKey);});
    document.getElementById('export-excel')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.specialties.export.excel") }}','excel'));
    document.getElementById('export-csv')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.specialties.export.csv") }}','csv'));
    document.getElementById('export-pdf')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.specialties.export.pdf") }}','pdf'));
    document.getElementById('export-print')?.addEventListener('click',()=>sendBulkRequest('{{ route("maintenance.specialties.print") }}','print'));
});
</script>
@endpush
