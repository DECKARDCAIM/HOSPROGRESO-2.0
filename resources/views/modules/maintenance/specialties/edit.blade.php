...
<main id="content" role="main" class="main"><div class="content container-fluid">
        ...
        <div class="col-sm mb-2 mb-sm-0"><nav aria-label="breadcrumb"><ol class="breadcrumb breadcrumb-no-gutter">
            ...
            <li class="breadcrumb-item"><a class="breadcrumb-link" href="{{ route('maintenance.specialties.index') }}">Especialidades</a></li>
            <li class="breadcrumb-item active" aria-current="page">Editar</li>
        </ol></nav><h1 class="page-header-title">Editar especialidad</h1></div>
        <div class="col-auto"><a href="{{ route('maintenance.specialties.index') }}" class="btn btn-primary"><i class="bi-arrow-left"></i> Regresar</a></div>
    </div></div>
@if ($errors->any())
    ...
    <div class="row justify-content-lg-center"><div class="col-lg-9">
            ...
            <form action="{{ route('maintenance.specialties.update', $item->id) }}" method="POST" id="itemForm" class="needs-validation" novalidate>
            @csrf @method('PUT')
            <div class="card card-lg mb-3 mb-lg-5">
                <div class="card-header"><h4 class="card-header-title">Detalles de la especialidad</h4></div>
                <div class="card-body"><div class="mb-4">
                    <label for="nameLabel" class="form-label">Nombre de la especialidad</label>
                    <div class="input-group input-group-merge">
                        <div class="input-group-prepend input-group-text"><i class="bi-heart-pulse"></i></div>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="nameLabel" placeholder="Ej. Cardiología" value="{{ old('name', $item->name) }}" required minlength="3" autocomplete="off">
                    </div>
                    @error('name')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                </div></div>
                <div class="card-footer d-flex justify-content-end align-items-center gap-3">
                    <a href="{{ route('maintenance.specialties.index') }}" class="btn btn-white">Cancelar</a>
