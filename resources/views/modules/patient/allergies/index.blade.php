        ...
        <div class="page-header">
            <div class="row align-items-end">
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="{{ route('allergies.create') }}">
                    ...
                    <form action="{{ route('allergies.index') }}" method="GET">
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            <a class="input-group-append input-group-text text-muted"
                                href="{{ route('allergies.index', request()->except('search')) }}">
                                <i class="bi-x-lg"></i>
                            </a>
                            @endif
                        </div>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-sm-end" aria-labelledby="allergiesExportDropdown">
                            ...
                            <a id="export-print" class="dropdown-item" href="javascript:;">
                                <img class="avatar avatar-xss avatar-4x3 me-2" src="{{ asset('svg/illustrations/print-icon.svg') }}" alt="Imprimir"> Imprimir
                            </a>
                                ...
                                <div class="card-body">
                                    <form action="{{ route('allergies.index') }}" method="GET">
                                        ...
                                        <div class="d-grid gap-2">
                                            ...
                                            <a class="btn btn-white" href="{{ route('allergies.index') }}">Limpiar</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('allergies.edit', $item->id) }}" class="btn btn-white btn-sm" title="Editar">
                                        <i class="bi-pencil-fill"></i>
                                    </a>
                                    @if ($item->is_active)
                                    <form action="{{ route('allergies.destroy', $item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @else
                                    <form action="{{ route('allergies.restore', $item->id) }}" method="POST">
                                        @csrf
                        ...
                        <div class="d-flex justify-content-center justify-content-sm-start align-items-center">
                            ...
                            <div class="tom-select-custom">
                                <select id="datatableEntries" class="js-select form-select form-select-borderless w-auto" autocomplete="off"
                                    ...
                                    onchange="window.location.href = '{{ route('allergies.index', request()->except(['per_page', 'page'])) }}' + ( '{{ route('allergies.index', request()->except(['per_page', 'page'])) }}'.includes('?') ? '&' : '?' ) + 'per_page=' + this.value">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.getElementById('bulk-delete')?.addEventListener('click', function() {
            sendBulkRequest('{{ route("allergies.destroy-multiple") }}');
            sessionStorage.removeItem(storageKey);
        });
        document.getElementById('bulk-restore')?.addEventListener('click', function() {
            sendBulkRequest('{{ route("allergies.restore-multiple") }}');
            sessionStorage.removeItem(storageKey);
        });
        document.getElementById('export-excel')?.addEventListener('click', () => sendBulkRequest('{{ route("allergies.export.excel") }}', 'excel'));
        document.getElementById('export-csv')?.addEventListener('click', () => sendBulkRequest('{{ route("allergies.export.csv") }}', 'csv'));
        document.getElementById('export-pdf')?.addEventListener('click', () => sendBulkRequest('{{ route("allergies.export.pdf") }}', 'pdf'));
        document.getElementById('export-print')?.addEventListener('click', () => sendBulkRequest('{{ route("allergies.print") }}', 'print'));
    });
</script>
@endpush
