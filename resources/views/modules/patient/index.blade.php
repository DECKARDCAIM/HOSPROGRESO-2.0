            ...
            <div class="card-header card-header-content-md-between">
                <div class="mb-2 mb-md-0">
                    <form id="searchForm" method="GET" action="{{ route('patients.index') }}">
                        ...
                        @endif
                        @if (request('gender_id')) <input type="hidden" name="gender_id"
                            value="{{ request('gender_id') }}"> @endif
                        @if (request('civil_status_id')) <input type="hidden" name="civil_status_id"
                            value="{{ request('civil_status_id') }}"> @endif
                        @if (request('ethnicity_id')) <input type="hidden" name="ethnicity_id"
                            value="{{ request('ethnicity_id') }}"> @endif
                        @if (request('linguistic_community_id')) <input type="hidden" name="linguistic_community_id"
                            value="{{ request('linguistic_community_id') }}"> @endif
                        @if (request('department_id')) <input type="hidden" name="department_id"
                            value="{{ request('department_id') }}"> @endif
                        @if (request('municipality_id')) <input type="hidden" name="municipality_id"
                            value="{{ request('municipality_id') }}"> @endif
                        ...
                        <div class="input-group input-group-merge input-group-flush">
                            ...
                            <a class="input-group-append input-group-text text-muted"
                                href="{{ route('patients.index', request()->except('search')) }}">
                                <i class="bi-x-lg"></i>
                            </a>
                            @endif
                        </div>
                ...
                <div class="d-grid d-sm-flex justify-content-md-end align-items-sm-center gap-2">
                    ...
                    <div class="dropdown">
                        <button type="button" class="btn btn-white btn-sm dropdown-toggle w-100"
                            id="usersExportDropdown" data-bs-toggle="dropdown" aria-expanded="false" title="Exportar">
                            <style>
                                    ...
                                    <form id="filterForm" method="GET" action="{{ route('patients.index') }}">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <select name="department_id" id="filter_department_id"
                                                    ...
                                                    <option value="">Todos</option>
                                                    @if (request('country_id'))
                                                    @foreach ($departments->where('country_id', request('country_id'))
                                                    as $department)
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <select name="municipality_id" id="filter_municipality_id"
                                                    ...
                                                    <option value="">Todos</option>
                                                    @if (request('department_id'))
                                                    @foreach ($municipalities->where('department_id',
                                                    request('department_id')) as $municipality)
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <select name="gender_id" id="filter_gender_id"
                                                    class="js-select form-select form-select-sm">
                                                    <option value="">Todos</option>
                                                    @foreach ($genders as $gender)
                                                    <option value="{{ $gender->id }}" {{ request('gender_id')==$gender->
                                                        id ? 'selected' : '' }}>
                                                        {{ $gender->name }}
                                                    </option>
                                                    @endforeach
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <select name="civil_status_id" id="filter_civil_status_id"
                                                    class="js-select form-select form-select-sm">
                                                    <option value="">Todos</option>
                                                    @foreach ($civilStatuses as $civilStatus)
                                                    <option value="{{ $civilStatus->id }}" {{
                                                        request('civil_status_id')==$civilStatus->id ? 'selected' : ''
                                                        }}>
                                                        {{ $civilStatus->name }}
                                                    </option>
                                                    @endforeach
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <select name="ethnicity_id" id="filter_ethnicity_id"
                                                    class="js-select form-select form-select-sm">
                                                    <option value="">Todas</option>
                                                    @foreach ($ethnicities as $ethnicity)
                                                    <option value="{{ $ethnicity->id }}" {{
                                                        request('ethnicity_id')==$ethnicity->id ? 'selected' : '' }}>
                                                        {{ $ethnicity->name }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-12 mb-3">
                                                ...
                                                <select name="linguistic_community_id"
                                                    id="filter_linguistic_community_id"
                                                    class="js-select form-select form-select-sm">
                                                    <option value="">Todas</option>
                                                    @foreach ($linguisticCommunities as $linguisticCommunity)
                                                    <option value="{{ $linguisticCommunity->id }}" {{
                                                        request('linguistic_community_id')==$linguisticCommunity->id ?
                                                        'selected' : '' }}>
                                                        {{ $linguisticCommunity->name }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        ...
                                        <div class="d-grid gap-2">
                                            <button type="submit" class="btn btn-primary btn-sm">Aplicar
                                                Filtros</button>
                                            <a href="{{ route('patients.index') }}" class="btn btn-white btn-sm">Limpiar
                                                Filtros</a>
                                        </div>
                                    </form>
                                </div>
                        ...
                        <tr>
                            <td class="table-column-pe-0">
                                <div class="form-check">
                                    <input class="form-check-input patient-checkbox" type="checkbox"
                                        name="patient_ids[]" value="{{ $patient->id }}"
                                        id="patientCheck{{ $patient->id }}">
                            ...
                            <td class="table-column-ps-0">
                                <a class="d-flex align-items-start" href="{{ route('patients.show', $patient) }}">
                                    <div class="avatar avatar-soft-primary avatar-circle flex-shrink-0">
                                        <span class="avatar-initials">{{ strtoupper(substr($patient->first_name ??
                                            ($patient->mother_first_name ?? 'P'), 0, 1)) }}</span>
                                    </div>
                                    <div class="ms-3 flex-grow-1">
                                        <span class="d-block h5 text-inherit mb-0">{{ $patient->full_name ?: 'Sin
                                            nombre' }}</span>
                                        <span class="d-block fs-5 text-body">{{ $patient->dpi ?: 'Sin DPI' }}</span>
                                    </div>
                                </a>
                            </td>
                            ...
                            <td>
                                <div class="btn-group" role="group">
                                    ...
                                    @else
                                    <a class="btn btn-white btn-sm" href="{{ route('patients.edit', $patient) }}"
                                        title="Editar">
